...
class ADODB_postgres64 extends ADOConnection{
    ...
    var $metaTablesSQL = "select tablename,'T',schemaname from pg_tables where tablename not like 'pg\_%'
...
// Warning: This compilation does not support pg_cmdtuples() in adodb-postgres.inc.php on line 44
   function _affectedrows()
   {
	...
	function MetaTables($ttype=false,$showSchema=false,$mask=false) 
	{
		...
		if ($info['version'] >= 7.3) {
		#fp19 modificado para ver los esquemas
		#original
	...
	union 
		...
		#modificado
		$this->metaTablesSQL = "select tablename,'T',schemaname from pg_tables where tablename not like 'pg\_%'
			  and schemaname  not in ( 'pg_catalog','information_schema')
	union 
        select viewname,'V',schemaname from pg_views where viewname not like 'pg\_%'  and schemaname  not in ( 'pg_catalog','information_schema') ";
		}
		if ($mask) {
