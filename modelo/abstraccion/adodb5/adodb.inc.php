...
*/
	...
	global $ADODB_FETCH_MODE;
		...
		if ($this->metaTablesSQL) {
			...
			for ($i=0; $i < sizeof($arr); $i++) {
				if ($hast) {
					if ($showt == 0) {
					#freddyp19 modificado para ver los schemas
					#origina 
					#if (strncmp($arr[$i][1],'T',1) == 0) $arr2[] = trim($arr[$i][0]);
					/*inicio de la modificacion*/	
						if ((strncmp($arr[$i][1],'T',1) == 0) && ($showSchema)) {
							$arr2[] = trim($arr[$i][2].'.'.$arr[$i][0]);
						}
						elseif (strncmp($arr[$i][1],'T',1) == 0) {
							$arr2[] = trim($arr[$i][0]);
						}
	...
	/**
	 * freddyp19 creado para detectar el schema de la tabla
	 * @param ttype can either be 'VIEW' or 'TABLE' or false. 
	 * 		If false, both views and tables are returned.
	 *		"VIEW" returns only views
	 *		"TABLE" returns only tables
	...
	global $ADODB_FETCH_MODE;
		...
		#	}
			...
			#	if ($hast) {
			#		if ($showt == 0) {
					#fp19 modificado para ver los schemas
					#origina 
			#		if (strncmp($arr[$i][1],'T',1) == 0) $arr2[] = trim($arr[$i][0]);
					/*inicio de la modificacion* /	
						if ((strncmp($arr[$i][1],'T',1) == 0) && ($showSchema)) {$arr2[] = trim($arr[$i][2].'.'.$arr[$i][0]);}
						else {$arr2[] = trim($arr[$i][0]);}
					/ *fin de la modificacion*/
			#		} else {
